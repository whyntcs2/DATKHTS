...
#define TANG_NANO_20K_CLK_FREQ 27000000
...
int main(int argc, char **argv)
{
  ...
  int idiv_sel = 0, fbdiv_sel = 0, odiv_sel = 32;
  ...
  board = argv[1];
  if (strcmp(board, "BOARD_20K") != 0) {
    fprintf(stderr, "This project supports only BOARD_20K\n");
    exit(EXIT_FAILURE);
  }

  target_clk_freq = atoi(argv[2]);
  if (target_clk_freq != TANG_NANO_20K_CLK_FREQ) {
    fprintf(stderr, "This project supports only a 27000000 Hz system clock\n");
    exit(EXIT_FAILURE);
  }

  clk_freq = target_clk_freq;
